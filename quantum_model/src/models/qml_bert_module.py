...
from src.models.components.qml_bert_pretrain_heads import BertPreTrainingHeads4QML, BertPreTrainingHeads
...
class QMLBertLitModule(LightningModule):
    ...
    #     input_ids, input_mask, segment_ids, lm_label_ids, is_next = batch
    #     loss, nsp_acc, mlm_loss, nsp_loss, _ = self(input_ids, segment_ids, input_mask, lm_label_ids, is_next, output_attentions=False)
    #     self.log('val_loss', loss.item(),sync_dist=True)
    #     self.log('val_nsp_acc', nsp_acc,sync_dist=True)
    #     self.log('val_masked_lm_loss', mlm_loss,sync_dist=True)
    #     self.log('val_next_sentence_loss', nsp_loss,sync_dist=True)
    #     return loss 
